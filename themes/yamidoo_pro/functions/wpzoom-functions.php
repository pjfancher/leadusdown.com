...
function zenko_popular ($args) { 
        ...
        $wrappeddate = date("F j, Y", mktime($datearray[3], $datearray[4], $datearray[5], $datearray[1], $datearray[2], $datearray[0]));
				...
				echo "<li><a href='".get_permalink($r->ID)."' rel='bookmark'>".htmlspecialchars($r->post_title, ENT_QUOTES)."</a>".htmlspecialchars($r->comment_count, ENT_QUOTES)." comments";
         				 echo"</li>\n";
...
function tabber_tabs_plugin_init() {
	...
	register_sidebar(
		array(
			...
			'before_widget' => '<div id="%1$s" class="tabbertab %2$s"><div class="tab_container">',
...
<ul>
	...
	<li>
		<?php unset($img); 
			...
			<a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>"><img src="<?php bloginfo('template_directory'); ?>/scripts/timthumb.php?src=<?php echo $img ?>&amp;w=50&amp;h=50&amp;zc=1" alt="<?php the_title(); ?>" /></a><?php } ?>
