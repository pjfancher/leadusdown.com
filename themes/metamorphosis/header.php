...
<head profile="http://gmpg.org/xfn/11">
	...
	<style type="text/css">h2, h3 { font-family: 'Droid Sans', sans-serif; }</style>     
        ...
        <ul id="pagenav">
			...
			if ( get_option('woo_custom_nav_menu') == 'true' ) {
        		if ( function_exists('woo_custom_navigation_output') )
					woo_custom_navigation_output('name=Woo Menu 1&depth=1');

			} else { ?>
            <li class="<?php if ( is_home() ) { echo 'current_page_item'; } ?>"><a href="<?php bloginfo('url'); ?>" title="<?php bloginfo('name'); ?>"><span class="left"></span><?php _e('Home',woothemes); ?><span class="right"></span></a></li>            
		...
		<div id="menu">
            ...
            <?php } ?>
             ...
             <form method="get" id="searchform" action="<?php bloginfo('home'); ?>/">                
                <div id="search">
                    <input type="text" value="search" onfocus="if (this.value == '<?php _e('search',woothemes); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e('search',woothemes); ?>';}" name="s" id="s" />
                    <input name="" type="hidden" value="<?php _e('Go',woothemes); ?>" class="btn"  />
                </div>                                                
            </form>
            <!-- end search -->  
